  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <AnimatePresence>{isOpen && (
        <motion.div initial={{ opacity: 0, scale: 0.85, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.85, y: 20 }} transition={{ duration: 0.4 }} className="absolute bottom-[72px] right-0 w-[300px] bg-white rounded-2xl p-5 mb-2 premium-shadow-lg border border-[#E2E8F0]/50">
          <div className="flex items-center justify-between mb-3">
            <h4 className="font-bold font-[family-name:var(--font-heading)] text-[#1A1A2E] text-[15px]">Chat with us</h4>
            <motion.button onClick={() => setIsOpen(false)} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="w-7 h-7 rounded-lg bg-[#1A3C6E]/[0.06] flex items-center justify-center"><X className="w-3.5 h-3.5 text-[#1A3C6E]" /></motion.button>
          </div>
          <p className="text-[13px] text-[#4A5568] mb-4 leading-relaxed">Hi there! How can we help you today?</p>
