...
export default function Contact() {
  const { t } = useLang();
  return (
    <section id="contact" className="section-padding bg-[#F7F9FC] relative overflow-hidden">
      <div className="max-w-[1400px] mx-auto relative z-10">
        <ScrollReveal>
          <div className="text-center max-w-3xl mx-auto mb-16 md:mb-20">
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#1A3C6E]/[0.05] text-[#1A3C6E] text-[12px] font-semibold mb-5 tracking-[2px] uppercase"><span className="w-1.5 h-1.5 rounded-full bg-[#1A3C6E] animate-pulse" />{t.contact.badge}</span>
            ...
            <div className="space-y-5">
              <StaggerChildren stagger={0.1}>
                {[{ icon: MapPin, title: t.contact.visit, lines: t.contact.address.split("\n") }, { icon: Phone, title: t.contact.call, lines: [t.contact.phone1, t.contact.phone2] }, { icon: Mail, title: t.contact.email, lines: [t.contact.emailAddr, t.contact.emailAddr2] }, { icon: Clock, title: t.contact.hours, lines: t.contact.hoursDetail.split("\n") }].map((info) => (
                  <StaggerItem key={info.title}><div className="flex gap-4 p-5 bg-white rounded-2xl premium-shadow border border-[#E2E8F0]/50 hover:premium-shadow-lg transition-all duration-500 group"><div className="w-12 h-12 rounded-xl bg-[#1A3C6E]/[0.06] flex items-center justify-center shrink-0 group-hover:bg-[#1A3C6E]/[0.1] transition-colors"><info.icon className="w-5 h-5 text-[#1A3C6E]" /></div><div><h4 className="font-bold font-[family-name:var(--font-heading)] text-[#1A1A2E] text-[15px] mb-1">{info.title}</h4>{info.lines.map((line, j) => <p key={j} className="text-[#4A5568] text-[14px] leading-relaxed">{line}</p>)}</div></div></StaggerItem>
                ))}
              </StaggerChildren>
            </div>
          </ScrollReveal>
          <ScrollReveal direction="right" distance={50}>
            <div className="bg-white rounded-3xl p-8 md:p-10 premium-shadow border border-[#E2E8F0]/50">
              <h3 className="text-[20px] font-bold font-[family-name:var(--font-heading)] text-[#1A1A2E] mb-6">{t.contact.badge}</h3>
              <form className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div><label className="block text-[12px] text-[#4A5568] mb-2 tracking-wide uppercase">Name</label><input type="text" className="w-full px-5 py-4 rounded-xl bg-[#F7F9FC] border border-[#E2E8F0] text-[#1A1A2E] placeholder-[#A0AEC0] focus:outline-none focus:ring-2 focus:ring-[#1A3C6E]/20 transition-colors text-[14px]" placeholder="Your name" /></div>
                  <div><label className="block text-[12px] text-[#4A5568] mb-2 tracking-wide uppercase">Email</label><input type="email" className="w-full px-5 py-4 rounded-xl bg-[#F7F9FC] border border-[#E2E8F0] text-[#1A1A2E] placeholder-[#A0AEC0] focus:outline-none focus:ring-2 focus:ring-[#1A3C6E]/20 transition-colors text-[14px]" placeholder="Your email" /></div>
                </div>
                <div><label className="block text-[12px] text-[#4A5568] mb-2 tracking-wide uppercase">Message</label><textarea rows={4} className="w-full px-5 py-4 rounded-xl bg-[#F7F9FC] border border-[#E2E8F0] text-[#1A1A2E] placeholder-[#A0AEC0] focus:outline-none focus:ring-2 focus:ring-[#1A3C6E]/20 transition-colors resize-none text-[14px]" placeholder="How can we help you?" /></div>
